...
struct MCPToolAnnotationTests {
    @Test func anUnannotatedToolIsNeverTreatedAsReadOnly() {
        let bare = MCPToolDescriptor(name: "run")
        let emptyAnnotations = MCPToolDescriptor(name: "run", annotations: MCPToolAnnotations())
        let titleOnly = MCPToolDescriptor(name: "run", annotations: MCPToolAnnotations(title: "Run"))
        let idempotentOnly = MCPToolDescriptor(name: "run", annotations: MCPToolAnnotations(idempotentHint: true))
    ...
    @Test func hintsMapToTheVerdictMCPItselfUses() {
        #expect(MCPToolDescriptor(name: "a", annotations: MCPToolAnnotations(readOnlyHint: true)).safety == .readOnly)
        // Read-only wins: MCP only reads destructiveHint when a tool is not read-only.
        #expect(
            MCPToolDescriptor(name: "b", annotations: MCPToolAnnotations(readOnlyHint: true, destructiveHint: true)).safety == .readOnly
        )
        // An explicit "not read-only" with no destructive hint keeps MCP's own default of destructive.
        #expect(MCPToolDescriptor(name: "c", annotations: MCPToolAnnotations(readOnlyHint: false)).safety == .destructive)
        #expect(MCPToolDescriptor(name: "d", annotations: MCPToolAnnotations(destructiveHint: false)).safety == .additive)
        #expect(MCPToolDescriptor(name: "e", annotations: MCPToolAnnotations(destructiveHint: true)).safety == .destructive)
        #expect(MCPToolDescriptor(name: "f", annotations: MCPToolAnnotations(readOnlyHint: true)).requiresRunConfirmation == false)
    }
    ...
    @Test func liveObservationCountsWhatAPersonNeedsToSeeBeforeRunningAnything() {
        let observation = MCPLiveObservation(
            ...
            tools: [
                MCPToolDescriptor(name: "read", annotations: MCPToolAnnotations(readOnlyHint: true)),
                MCPToolDescriptor(name: "wipe", annotations: MCPToolAnnotations(destructiveHint: true)),
                MCPToolDescriptor(name: "mystery"),
            ],
            resourceCount: 0,
            promptCount: 0,
            declaresTools: true,
