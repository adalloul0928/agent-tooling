            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    SettingsGroup(title: "Local workspace", symbol: "externaldrive") {
                        ...
                        if let preview = model.backupImportPreview {
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                HStack {
                                    Label(
                                        preview.conflicts.isEmpty ? "No desired-state conflicts" : "\(preview.conflicts.count) conflicts",
                                ...
                                Text(preview.backupURL.path(percentEncoded: false)).font(.caption.monospaced()).foregroundStyle(.secondary)
                                    .lineLimit(1)
                    ...
                    SettingsGroup(title: "Encrypted folder sync", symbol: "lock.arrow.triangle.2.circlepath") {
                        SettingsValueRow(title: "Encrypted archive", detail: model.encryptedSyncConfiguration.location ?? "Not configured")
                        {
                        ...
                        if let preview = model.encryptedSyncImportPreview {
                            ...
                            HStack(spacing: 10) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    Text("Decrypted preview: \(preview.libraryFileCount) managed files").font(.caption.weight(.semibold))
                        ...
                        if model.managedPolicies.isEmpty {
                            ...
                            ForEach(model.managedPolicies) { policy in
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text(policy.sourcePath).font(.caption2.monospaced()).foregroundStyle(.tertiary).lineLimit(1)
                                }
                                .padding(14)
                            }
                        }
                    ...
                    SettingsGroup(title: "Behavior", symbol: "switch.2") {
                        ...
                        Divider()
                        SettingsToggleRow(
                            title: "Show menu bar item", detail: "Keep setup checks and sync actions one click away", isOn: $showMenuBarItem
                        )
                        Divider()
                        SettingsToggleRow(
                    ...
                    SettingsGroup(title: "Appearance", symbol: "circle.lefthalf.filled") {
                        HStack {
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                Text("The window material adapts to the selected system style; controls remain native and readable.").font(
                                    .caption
                                ).foregroundStyle(.secondary)
                            }
                            Spacer()
