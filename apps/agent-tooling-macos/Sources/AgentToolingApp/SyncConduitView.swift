...
struct SyncConduitView: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(
            "Sync path from the library through \(profileName) to \(terminals.count) clients")
    }

    private var connectorHeight: CGFloat {
        CGFloat(terminals.count) * terminalHeight
            + CGFloat(max(0, terminals.count - 1)) * terminalSpacing
    }

    private var libraryDetail: String {
        discoveredCount > 0
            ? "\(managedCount) managed · \(discoveredCount) discovered" : "\(managedCount) managed"
    }
    ...
    private func node(kind: ToolingKind, title: String, detail: String, action: @escaping () -> Void)
        -> some View
    {
        ...
        GeometryReader { geometry in
            ...
            ZStack(alignment: .bottom) {
                TimelineView(.animation(paused: !animate)) { context in
                    let phase =
                        animate
                        ? -CGFloat(context.date.timeIntervalSinceReferenceDate * 16)
                            .truncatingRemainder(dividingBy: 18)
                        : 0
                    Path { path in
                    ...
                    .stroke(
                        ...
                        style: StrokeStyle(
                            lineWidth: 1.75, lineCap: .round, dash: pending ? [4, 5] : [],
                            dashPhase: phase)
                    )
                    .animation(lineAnimation, value: revealed)
                }
                ...
                ForEach(Array(terminals.enumerated()), id: \.element.id) { index, terminal in
                    ...
                    TimelineView(.animation(paused: !(attention && animate))) { context in
                        ...
                        .stroke(
                            ...
                            style: StrokeStyle(
                                lineWidth: 1.5, lineCap: .round, dash: attention ? [4, 5] : [],
                                dashPhase: phase)
                        )
        ...
        Button(action: action) {
            ...
            .background {
                Capsule()
                    ...
                    .shadow(
                        color: .black.opacity(hovering ? 0.14 : 0.05), radius: hovering ? 6 : 1,
                        y: hovering ? 3 : 1)
            }
            .overlay {
                Capsule().strokeBorder(
                    AgentTheme.separator.opacity(hovering ? 0.8 : 0.5), lineWidth: 0.5)
            }
...
extension View {
    ...
    fileprivate func revealed(_ revealed: Bool, delay: Double, reduceMotion: Bool) -> some View {
