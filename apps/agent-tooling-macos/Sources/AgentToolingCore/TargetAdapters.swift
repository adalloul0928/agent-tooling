...
public protocol TargetAdapter: Sendable {
...
private enum LocalTargetScan {
    ...
    private static func pluginNames(from configuration: String) -> [String] {
        ...
        for line in configuration.split(separator: "\n") {
            guard let section = tomlTableHeader(String(line)),
                let path = tomlKeyPath(section),
                path.count == 2,
                path[0] == "plugins",
                !path[1].isEmpty
            else { continue }
    ...
    private static func mcpRecords(from configuration: String, source: String) -> [String: ObservedMCPMetadata] {
        ...
        for line in configuration.split(separator: "\n") {
            guard let section = tomlTableHeader(String(line)) else { continue }
            guard let path = tomlKeyPath(section), path.count == 2,
    ...
    private static func tomlTableHeader(_ line: String) -> String? {
        var quote: Character?
        var escaping = false
        var content = ""
    ...
    private static func tomlKeyPath(_ rawValue: String) -> [String]? {
        var result: [String] = []
        var current = ""
        var quote: Character?
        var escaping = false
