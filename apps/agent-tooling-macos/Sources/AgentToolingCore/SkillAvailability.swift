...
enum SkillAvailability {
    ...
    static func json(_ data: Data, key: String, identifier: String, enabled: Bool?) throws -> (Bool, Data) {
        ...
        var entries = root[key] as? [String: Any] ?? [:]
        let current: Bool
        if key == "enabledPlugins" {
            current = entries[identifier] as? Bool ?? true
        } else {
            current = entries[identifier] as? String != "off"
        }
        if let enabled {
            entries[identifier] = key == "enabledPlugins" ? enabled as Any : (enabled ? "on" : "off") as Any
            root[key] = entries
        }
        return (current, try JSONSerialization.data(withJSONObject: root, options: [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]))
    }
    ...
    static func codex(_ text: String, path: String, enabled: Bool?) throws -> (Bool, String) {
        ...
        let incompatible = try NSRegularExpression(pattern: #"(?m)^\s*(?:skills\s*\.\s*config\s*=|\[skills\.config\]|config\s*=)"#)
        guard incompatible.firstMatch(in: text, range: NSRange(text.startIndex..., in: text)) == nil else {
            throw Failure.unsupported("This Codex file uses a different skill configuration format. Edit availability in Codex instead.")
        }
        ...
        for (offset, start) in headers.enumerated() {
            ...
            for index in (start + 1)..<end {
                ...
                guard let range = regex.firstMatch(in: raw, range: NSRange(raw.startIndex..., in: raw))?.range,
                    let swiftRange = Range(range, in: raw),
                    let parsed = try? JSONDecoder().decode(String.self, from: Data(raw[swiftRange].utf8))
                else {
                    throw Failure.unsupported("A Codex skill path uses unsupported TOML syntax. Use Codex's Skills controls for this file.")
                }
                if parsed == path {
        ...
        if let match {
            ...
            if let index = entries.first {
                ...
                current = value == "true"
                if let enabled { lines[index] = "enabled = \(enabled)" }
            } else if let enabled {
                lines.insert("enabled = \(enabled)", at: match.lowerBound + 1)
            }
        } else if let enabled {
            let encoder = JSONEncoder()
            encoder.outputFormatting = [.withoutEscapingSlashes]
            let quoted = String(decoding: try encoder.encode(path), as: UTF8.self)
            lines.append(contentsOf: ["", "[[skills.config]]", "path = \(quoted)", "enabled = \(enabled)", ""])
        }
        return (current, lines.joined(separator: "\n"))
    }
}
