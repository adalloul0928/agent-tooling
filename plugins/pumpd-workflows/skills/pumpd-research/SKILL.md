...
Stage 1 of the PUMPD planning pipeline: **research → plan → decompose → Cyrus**.
...
## Inputs
- A rough idea (from chat, or an existing note in `PUMPD/Tasks/Todo/`).
...
## Steps
1. **Locate/capture the idea.** If it isn't already an idea note, optionally capture it to `PUMPD/Tasks/Todo/<Idea>.md` using `PUMPD/_Templates/Idea Template.md`. Preserve the user's raw wording.
...
5. **Check the vault** (`PUMPD/Research`, `PUMPD/Tasks`, `PUMPD/Decisions`, `PUMPD/Operations`) so you build on prior decisions and don't duplicate. Cross-link with `[[wikilinks]]`.
...
8. **Write the research note** to `PUMPD/Research/<Feature> — Research.md` using `PUMPD/_Templates/Research Template.md`. Fill: Idea, Codebase findings, External research (every claim with a **direct URL**), Constraints, Approaches considered, **Decision + why**, a consolidated **Sources** list (official docs · recent repos w/ versions · releases), Open questions. Frontmatter `type: research`, `status: draft`, tags `[pumpd, research, <domain>]`. These source links must survive into the plan's §4 — capture them well.
