...
| Sentry → stdio token via Doppler | A | blocked on owner | Owner mints `SENTRY_ACCESS_TOKEN` into Doppler `agent-tooling/prd`; then add `@sentry/mcp-server` block to `plugins/mobile-development/.mcp.json`; remove OAuth-remote connector; add profile check |
