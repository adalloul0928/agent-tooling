...
| Expo MCP (Claude; Codex deferred) | A | landed · owner OAuth | Claude side landed as `claude.expo-mcp` in `base-workstation`, recipe `claude mcp add --scope user --transport http expo https://mcp.expo.dev/mcp`. **Codex deferred 2026-07-23** to a dedicated pass: `pumpd-workstation` asserts `codex.duplicate-expo-mcp` absent (Codex gets expo from `openai-curated`), so a `present` check would contradict it — that pass settles curated-vs-raw for expo and sentry together. OAuth-only, no PAT/CI fallback — never relied on in CI/cron |
