...
| Sentry → hosted OAuth remote (`mcp.sentry.dev`) | A | **done** | **Decision revised 2026-07-23:** owner chose Sentry's hosted remote MCP over the stdio-token approach. Added per client via `claude mcp add --transport http` / `codex mcp add --url` (project-scoped `avad-technologies-llc/pumpd`), tracked as `claude.sentry-mcp` / `codex.sentry-mcp` in `base-workstation`. Local OAuth, reused by local scheduled runs; no detached-cloud/headless path. Owner completes the browser OAuth per client |
